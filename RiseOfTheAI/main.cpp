...
const char  SPRITESHEET_FILEPATH[]  = "assets/male.png",
            TILESET_FILEPATH[]      = "assets/tilemap_packed.png",
            GUARD_FILEPATH[]        = "assets/guard.png",
            WALKER_FILEPATH[]       = "assets/walker.png",
            JUMPER_FILEPATH[]       = "assets/jumper.png",
            FONT_FILEPATH[]         = "assets/font1.png";


const char  BGM_FILEPATH[]          = "assets/music.mp3";
...
void initialise()
{
    ...
    g_game_state.map = new Map(LEVEL1_WIDTH, LEVEL1_HEIGHT, LEVEL_1_DATA, map_texture_id, 1.0f, 16, 7);
    ...
    g_game_state.player->set_speed(2.75f);
    ...
    // Jumping
    g_game_state.player->set_jumping_power(4.75f);

    // ––––– ENEMIES ––––– //
    GLuint guard_texture_id = load_texture(GUARD_FILEPATH);
    GLuint walker_texture_id = load_texture(WALKER_FILEPATH);
    ...
    for (int i = 0; i < 2; i++){
        g_game_state.enemies[i].m_walking[g_game_state.enemies[i].LEFT]   = new int[3] { 3, 4, 5 };
        g_game_state.enemies[i].m_walking[g_game_state.enemies[i].RIGHT]  = new int[3] { 6, 7, 8 };
        g_game_state.enemies[i].m_animation_indices = g_game_state.enemies[i].m_walking[g_game_state.enemies[i].RIGHT];
        g_game_state.enemies[i].m_animation_frames  = 3;
    ...
    g_game_state.enemies[0].m_texture_id = guard_texture_id;
    g_game_state.enemies[0].set_position(glm::vec3(6.0f, -3.0f, 0.0f));
    g_game_state.enemies[0].set_speed(0.5f);

    
    g_game_state.enemies[1].set_ai_type(WALKER);
    g_game_state.enemies[1].set_ai_state(ROAM_RIGHT);
    g_game_state.enemies[1].m_texture_id = walker_texture_id;
    g_game_state.enemies[1].set_position(glm::vec3(9.0f, 0.0f, 0.0f));
    ...
    g_game_state.enemies[2].m_texture_id = jumper_texture_id;
    g_game_state.enemies[2].set_position(glm::vec3(17.0f, 2.0f, 0.0f));
    g_game_state.enemies[2].set_jumping_power(2.5f);
...
void render()
{
    ...
    if (g_game_state.player->m_enemies_killed == ENEMY_COUNT){
        draw_text(&g_shader_program, font_texture_id, std::string("YOU WIN"), 0.75f, 0.0f, glm::vec3(g_game_state.player->get_position().x - 2.25f, g_game_state.player->get_position().y + 1.0f, 0.0f));
    }
    else if (g_game_state.player->m_is_dead){
        draw_text(&g_shader_program, font_texture_id, std::string("YOU LOSE"), 0.75f, 0.0f, glm::vec3(g_game_state.player->get_position().x - 2.5f, g_game_state.player->get_position().y + 1.0f, 0.0f));
    }
